...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:util="http://hl7.nist.gov/data-specs/util"
	xmlns="http://www.w3.org/1999/xhtml" xpath-default-namespace="" exclude-result-prefixes="xs"
	version="2.0">
	...
	<xsl:template match="PID">
		...
		<xsl:value-of
			select="util:element('Patient Name', concat(util:format-with-space(.//PID.5.2), util:format-with-space(.//PID.5.3), .//PID.5.1.1), $ind1)"/>
		<xsl:value-of
			select="util:element('Mother''s Maiden Name', concat(util:format-with-space(.//PID.6.2), .//PID.6.1.1), $ind1)"/>
		<xsl:value-of
			select="util:element('ID Number', util:format-with-space(.//PID.3[1]/PID.3.1), $ind1)"/>
		<xsl:value-of
		...
		<xsl:for-each select=".//PID.11">
			<xsl:value-of
				select="util:element(concat('Patient Address', ' ', position()), concat(util:format-address(PID.11.1/PID.11.1.1, PID.11.3, PID.11.4, PID.11.5), PID.11.6), $ind1)"
			/>
		</xsl:for-each>
		<xsl:value-of
			select="util:element('Local Number', util:format-tel(.//PID.13.6, .//PID.13.7), $ind1)"/>
	...
	<xsl:template match="QPD">
		...
		<xsl:value-of
			select="util:element('Patient Address', util:format-address(.//QPD.8.1.1, .//QPD.8.3, .//QPD.8.4, ''), $ind1)"/>
		<xsl:value-of
			select="util:last-element('Patient Phone', util:format-tel(.//QPD.9.6, .//QPD.9.7), $ind1)"
		/>
	</xsl:template>
	...
	<xsl:template match="PD1">
		<xsl:value-of
			select="util:title('title', 'Immunization Registry Information', $ind1, true())"/>
		<xsl:value-of select="util:elements($ind1)"/>
		...
		<xsl:value-of
			select="util:element('Protection Indicator Effective Date', util:format-date(.//PD1.13.1), $ind1)"/>
		<xsl:value-of
			select="util:element('Publicity Code Effective Date', util:format-date(.//PD1.18), $ind1)"/>
		<xsl:value-of
			select="util:last-element('Immunization Registry Status Effective Date', util:format-date(.//PD1.17), $ind1)"
		/>
	</xsl:template>
	...
	<xsl:template match="NK1">
		...
		<xsl:value-of select="util:elements($ind1)"/>
		<xsl:value-of
			select="util:element('Name', concat(util:format-with-space(.//NK1.2.2), .//NK1.2.1.1), $ind1)"/>
		<xsl:value-of select="util:element('Relationship', .//NK1.3.2, $ind1)"/>
		<xsl:value-of
			select="util:element('Address', util:format-address(.//NK1.4.1.1, .//NK1.4.3, .//NK1.4.4, .//NK1.4.5), $ind1)"/>
		<xsl:value-of select="util:element('Address (Country)', .//NK1.4.6, $ind1)"/>
		<xsl:value-of
			select="util:last-element('Phone Number or Email address', concat(util:format-with-space(.//NK1.5.4), util:format-tel(.//NK1.5.6, .//NK1.5.7)), $ind1)"
		/>
	</xsl:template>
	...
	<xsl:template match="RXA">
		<xsl:value-of
			select="util:title('title', 'Vaccine Administration Information', $ind1, true())"/>
		<xsl:value-of select="util:elements($ind1)"/>
		<xsl:value-of select="util:element('Administered Code - Text', .//RXA.5.2, $ind1)"/>
		<xsl:value-of
			select="util:element('Date/Time Start of Administration', util:format-date(.//RXA.3.1), $ind1)"/>
		...
		<xsl:value-of
			select="util:element('Administering Provider', concat(util:format-with-space(.//RXA.10.3), .//RXA.10.2.1), $ind1)"/>
		<xsl:value-of select="util:element('Substance Lot Number', .//RXA.15, $ind1)"/>
		<xsl:value-of
			select="util:element('Substance Expiration Date', util:format-date(.//RXA.16.1), $ind1)"/>
		<xsl:value-of select="util:element('Substance Manufacturer Name', .//RXA.17.2, $ind1)"/>
		<xsl:value-of
			select="util:element('Substance/Treatment Refusal Reason', .//RXA.18.2, $ind1)"/>
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of
					select="concat($nl, $indent, util:end-elements($ind2), $ind2, '}', $nl, $ind2, '}', $nl, $ind1, ']', $nl)"
				/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="ORC">
		...
		<xsl:value-of select="util:elements($ind1)"/>
		<xsl:value-of
			select="util:element('Entered By', concat(util:format-with-space(.//ORC.10.3), .//ORC.10.2.1), $ind1)"/>
		<xsl:value-of
			select="util:last-element('Ordered By', concat(util:format-with-space(.//ORC.12.3), .//ORC.12.2.1), $ind1)"
		/>
	</xsl:template>
	...
	<xsl:template name="test-story"> document.write('<div id="test-tabs-0">'); document.write('
			............. Test .......... Story................... '); document.write('</div>'); </xsl:template>

	<xsl:template name="message-content"> document.write('<div id="test-tabs-2">'); document.write('
			............. Message .......... Content................... ');
		document.write('</div>'); </xsl:template>

	<xsl:template name="test-data-specs"> document.write('<div id="test-tabs-1">');
				document.write('<div id="test-data-tabs">'); <xsl:if test="$output = 'tab-html'">
						document.write('<ul>'); document.write('<li><a href="#test-data-tabs-0"
								>FULL</a></li>'); for(var key in data.tables) {
								document.write('<li><a href="#test-data-tabs-' + (key+1) + '">' +
								data.tables[key].title + '</a></li>'); } document.write('</ul>');
				</xsl:if> for(var key in data.tables) { var tab = ''; tab += '<div
					id="test-data-tabs-' + (key+1) + '">'; var table = data.tables[key]; tab +=
							('<fieldset><legend>' + table.title + '</legend>
						<table>
							<tr>
								<th> Element </th>
								<th> Data </th>
							</tr>'); for (var elkey in table.elements) { element =
							table.elements[elkey]; // display obxs as separate table if
							(element.element == 'obx') { var obx = element.data; tab +=
						'</table></fieldset>'; // end the bigger table tab += ('<fieldset><table>
							<tr>
								<th colspan="2"> ' + obx.title + '</th>
							</tr>'); for (var obxkey in obx.elements) { if
							(obx.elements[obxkey].element == "") { // gray line tab += '<tr
								class="tds_obxGrpSpl"><td colspan="2"/></tr>'; } else { tab +=
									('<tr><td>' + obx.elements[obxkey].element + '</td><td>' +
									obx.elements[obxkey].data + '</td></tr>'); } } } else { var
							tdclass = element.data == '' ? "tds_noData" : "tds_data"; tab +=
									('<tr><td>' + element.element + '</td><td
									class="' + tdclass + '">' + element.data + '</td></tr>'); } }
							tab += '</table></fieldset>'; tab += '</div>'; <xsl:if
					test="$output = 'tab-html'"> document.write(tab); </xsl:if> full += tab;
				//$(function() { $( "#test-data-tabs-0" ).html("full"); }); } // full tab
					document.write('<div id="test-data-tabs-0"> ' + full + '</div>');
				document.write('</div>'); document.write('</div>'); </xsl:template>

	<xsl:template name="css">
		<style type="text/css">
			@media screen{
			    .test-data-specs-main .tds_obxGrpSpl{
			        background:#B8B8B8;
			    }
			    .test-data-specs-main maskByMediaType{
			        display:table;
			    }
			    .test-data-specs-main fieldset table tbody tr th{
			        font-size:95%
			    }
			    .test-data-specs-main fieldset table tbody tr td{
			        font-size:100%;
			    }
			    .test-data-specs-main fieldset table tbody tr th{
			        text-align:left;
			    ...
			    .test-data-specs-main fieldset table thead tr th{
			        text-align:center;
			    }
			    .test-data-specs-main fieldset{
			        text-align:center;
			    }
			    .test-data-specs-main fieldset table{
			        width:98%;
			        border:1px groove;
			        table-layout:fixed;
			        margin:0 auto;
			    ...
			    .test-data-specs-main fieldset table tr{
			        border:3px groove;
			    }
			    .test-data-specs-main fieldset table th{
			        border:2px groove;
			    }
			    .test-data-specs-main fieldset table td{
			        border:2px groove;
			    }
			    .test-data-specs-main fieldset table thead{
			        border:1px groove;
			        background:#446BEC;
			        text-align:left;
			    }
			    ...
			    .test-data-specs-main .tds_noData{
			        background:#B8B8B8;
			    }
			    .test-data-specs-main .tds_childField{
			        background:#B8B8B8;
			    }
			    .test-data-specs-main .tds_title{
			        text-align:left;
			    }
			    .test-data-specs-main h3{
			        text-align:center;
			        page-break-inside:avoid;
			    }
			    .test-data-specs-main h2{
			        text-align:center;
			    }
			    .test-data-specs-main h1{
			        text-align:center;
			    }
			    .test-data-specs-main .tds_pgBrk{
			        padding-top:15px;
			    }
			    .test-data-specs-main .tds_er7Msg{
			        width:100%;
			    }
			    .test-data-specs-main .tds_embSpace{
			        padding-left:15px;
			    }
			}
			...
			@media print{
			    ...
			    .test-data-specs-main tds_maskByMediaType{
			        display:table;
			    }
			    .test-data-specs-main fieldset table tbody tr th{
			        font-size:90%
			    ...
			    .test-data-specs-main fieldset table[id = vendor-labResults] thead tr{
			        font-size:80%
			    }
			    .test-data-specs-main fieldset table[id = vendor-labResults] tbody tr{
			        font-size:75%
			    }
			    .test-data-specs-main fieldset table tr{
			        border:3px groove;
			    ...
			    .test-data-specs-main fieldset#tds_er7Message table{
			        border:0px;
			        width:80%
			    }
			    .test-data-specs-main fieldset#tds_er7Message td{
			        background:#B8B8B8;
			        font-size:65%;
			        margin-top:6.0pt;
			        border:0px;
			    ...
			    .test-data-specs-main .tds_er7MsgNote{
			        width:100%;
			        font-style:italic;
			        font-size:80%;
			    }
			    ...
			    .test-data-specs-main .tds_embSubSpace{
			        padding-left:25px;
			    }
			}</style>
	</xsl:template>
	...
	<xsl:function name="util:start">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of
					select="concat('{', $nl, '&quot;profile&quot; : &quot;', $profile, '&quot;,', $nl, '&quot;tables&quot;:', $nl, '[', $nl)"
				/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:function>
	...
	<xsl:function name="util:begin-obx-table">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of
					select="concat($nl, $ind, '{&quot;element&quot; : &quot;obx&quot;, &quot;data&quot; : ', $nl)"
				/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:function>
		...
		<xsl:choose>
			<xsl:when test="$output = 'plain-html'">
				...
				<xsl:choose>
					...
					<xsl:when test="$value = ''">
						<xsl:value-of select="util:tags('legend', '', $ind)"/>
					</xsl:when>
				</xsl:choose>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:function name="util:elements">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of
					select="concat($ind, $indent, '&quot;elements&quot; : ', $nl, $ind, $indent, '[')"
				/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:function>
	...
	<xsl:function name="util:last-element">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of
					select="concat(util:element-with-delimiter($name, $value, '', $ind), $nl, $ind, $indent, ']', $nl)"
				/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:function>
