...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="QPD">
		<xsl:value-of select="util:title('title', 'Patient Information', $ind1, false())" />
		<xsl:value-of select="util:elements($ind1)" />
		<xsl:value-of select="util:element('Patient Name', concat(util:format-with-space (.//QPD.4.2), util:format-with-space(.//QPD.4.3), .//QPD.4.1), $ind1)" />
		...
		<xsl:value-of select="util:element('Patient Address', util:format-address(.//QPD.8.1, .//QPD.8.3, .//QPD.8.4, ''), $ind1)" />
	...
	<xsl:template match="PD1">
		...
		<xsl:value-of select="util:element('Publicity Code Effective Date', util:format-date(.//PD1.18.1), $ind1)" />
		<xsl:value-of select="util:last-element('Immunization Registry Status Effective Date', util:format-date(.//PD1.17.1), $ind1)" />
	</xsl:template>
	...
	<xsl:template match="RXA">
		<xsl:value-of select="util:title('title', 'Vaccine Adminstration Information', $ind1, true())" />
		<xsl:value-of select="util:elements($ind1)" />
	...
	<xsl:function name="util:title">
		...
		<xsl:variable name="prelude">
			<xsl:choose>
				<xsl:when test="$endprevioustable"><xsl:value-of select="concat($ind, '},', $nl)"/></xsl:when>
				<xsl:otherwise>
	...
	<xsl:function name="util:html-prelude">
	</xsl:function>

	<xsl:function name="util:html-footer">
	</xsl:function>
