...
//	private TreeNode messageTreeRoot = new DefaultTreeNode("root", null);
	...
	public void shareInit(ActionEvent event){
		try{
			this.shareTo = null;
			SimpleDataInstanceTestPlan simple = (SimpleDataInstanceTestPlan) event.getComponent().getAttributes().get("testplan");
			this.selectedTestPlan = this.sessionBeanTCAMT.getDbManager().getDataInstanceTestPlanById(simple.getId());
		}catch(Exception e){
	...
	public void cloneTestPlan(ActionEvent event)  {
		try{
			SimpleDataInstanceTestPlan simple = (SimpleDataInstanceTestPlan) event.getComponent().getAttributes().get("testplan");
			DataInstanceTestPlan testplan = this.sessionBeanTCAMT.getDbManager().getDataInstanceTestPlanById(simple.getId()).clone();
	...
	public void deleteTestPlan(ActionEvent event) {
		try{
			SimpleDataInstanceTestPlan simple = ((SimpleDataInstanceTestPlan) event.getComponent().getAttributes().get("testplan"));
			String deletedTestPlanName = simple.getName();
			DataInstanceTestPlan tp = this.sessionBeanTCAMT.getDbManager().getDataInstanceTestPlanById(simple.getId());
			this.sessionBeanTCAMT.getDbManager().dataInstanceTestPlanDelete(tp);
			this.sessionBeanTCAMT.updateDataInstanceTestPlans();
...
//	}
	...
	public void downloadTestPlanJSON(SimpleDataInstanceTestPlan simple) {
		try{
			DataInstanceTestPlan  tp = this.sessionBeanTCAMT.getDbManager().getDataInstanceTestPlanById(simple.getId());
			this.setDitpConverter(new JsonDataInstanceTestPlanConverter());
	...
	public void downloadResourceBundleForTestPlan(SimpleDataInstanceTestPlan simple) {
		try{
			DataInstanceTestPlan  tp = this.sessionBeanTCAMT.getDbManager().getDataInstanceTestPlanById(simple.getId());
...
//		packageBodyHTML = packageBodyHTML + "<p>Date : " + tp.getLastUpdateDate() + "</p>" + System.getProperty("line.separator");
	...
	private void createTCAMTConstraint(Object model, long codeListId) {
		try{
			String ipath = null;
			String data = null;
			...
			if(tdc != null && !tdc.getValue().equals("")){
				...
				if(tdc.equals(TestDataCategorization.Value_TestCaseFixedList)){
					...
					TestCaseCodeList tccl = this.sessionBeanTCAMT.getDbManager().getTestCaseCodeListById(codeListId);
	...
	public void processGlobalTDCSetting(){
		String segmentName = this.tdcGlobalSettingData.getPath().substring(0, 3);
		for(InstanceSegment is:this.instanceSegments){
			if(segmentName.equals(is.getPath().substring(is.getPath().length() - 3))){
				String path = this.tdcGlobalSettingData.getPath().replace("-", ".");
				...
				for(TreeNode fNode:this.segmentTreeRoot.getChildren()){					
					if(paths.length == 2){	
						...
						if(fModel.getPath().contains(path)){
							if(this.tdcGlobalSettingData.getTdc() != null){
								fModel.setTdc(this.tdcGlobalSettingData.getTdc());
								this.createTCAMTConstraint(fModel, this.tdcGlobalSettingData.getCodeListId());
							}
						}
						...
						for(TreeNode cNode:fNode.getChildren()){
							if(paths.length == 3){	
								...
								for(TreeNode scNode:cNode.getChildren()){
									...
									if(scModel.getPath().contains(path)){
										if(this.tdcGlobalSettingData.getTdc() != null){
											scModel.setTdc(this.tdcGlobalSettingData.getTdc());
											this.createTCAMTConstraint(scModel, this.tdcGlobalSettingData.getCodeListId());
										}
									}
								}
							}
	...
	private boolean isLeafNode(String path,
			gov.nist.healthcare.tools.hl7.v2.tcamt.lite.domain.profile.Message messageObj, Segments segments,
			Datatypes datatypes) {
	...
	private Object findChild(String location, Object parentObj, Segments segments, Datatypes datatypes){
		...
		if(parentObj instanceof gov.nist.healthcare.tools.hl7.v2.tcamt.lite.domain.profile.Message){
			gov.nist.healthcare.tools.hl7.v2.tcamt.lite.domain.profile.Message messageObj = (gov.nist.healthcare.tools.hl7.v2.tcamt.lite.domain.profile.Message)parentObj;
...
//			this.messageTreeRoot = this.manageInstanceService.loadMessage(this.selectedTestStep.getMessage());
			...
			for(DataInstanceTestCase ditc:ditg.getTestcases()){
				...
				for(DataInstanceTestStep dits:ditc.getTeststeps()){
					...
					if(dits != null && dits.getMessage() != null && dits.getMessage().getConformanceProfile() != null){
						...
						this.generateMessageContentForPDF(out, dits.getMessage(), teststepPath);
						...
						this.generateTestDataSpecificationForPDF(out, dits.getMessage(), teststepPath);
						this.generateTestDataSpecificationJSONRB(out, dits.getMessage(), teststepPath);
						this.generateJurorDocumentRB(out, dits.getMessage(), teststepPath, tp);
	...
	private void generateJurorDocumentRB(ZipOutputStream out, Message m, String path, DataInstanceTestPlan tp) throws IOException{
		if(tp.isJurorDocumentEnable()){
	...
	private void generateTestDataSpecificationForPDF(ZipOutputStream out, Message m, String path) throws IOException{
		if(m.getConformanceProfile().getTestDataSpecificationXSLT() != null){
			InputStream xsltInputStream = new ByteArrayInputStream(m.getConformanceProfile().getTestDataSpecificationXSLT().replaceAll("<xsl:param name=\"output\" select=\"'ng-tab-html'\"/>", "<xsl:param name=\"output\" select=\"'plain-html'\"/>").getBytes());
			InputStream sourceInputStream = new ByteArrayInputStream(m.getXmlEncodedNISTMessage().getBytes());
			Reader xsltReader =  new InputStreamReader(xsltInputStream, "UTF-8");
			Reader sourceReader = new InputStreamReader(sourceInputStream, "UTF-8");
			String xsltStr = IOUtils.toString(xsltReader);
			...
			if(testDataSpecificationHTMLStr != null && !testDataSpecificationHTMLStr.equals("")){
				...
				out.putNextEntry(new ZipEntry(path + File.separator + "TestDataSpecificationPDF.html"));
	...
	private void generateMessageContentForPDF(ZipOutputStream out, Message m, String path) throws Exception{
		if(m.getConformanceProfile().getMessageContentXSLT() != null){
			this.manageInstanceService.generateXMLforMessageContentAndUpdateTestData(m, this.instanceSegments);
			InputStream xsltInputStream = new ByteArrayInputStream(m.getConformanceProfile().getMessageContentXSLT().replaceAll("<xsl:param name=\"output\" select=\"'ng-tab-html'\"/>", "<xsl:param name=\"output\" select=\"'plain-html'\"/>").getBytes());
			InputStream sourceInputStream = new ByteArrayInputStream(m.getXmlEncodedMessageContent().getBytes());
			Reader xsltReader =  new InputStreamReader(xsltInputStream, "UTF-8");
			Reader sourceReader = new InputStreamReader(sourceInputStream, "UTF-8");
			String xsltStr = IOUtils.toString(xsltReader);
			...
			if(messageContentHTMLStr != null && !messageContentHTMLStr.equals("")){
				...
				out.putNextEntry(new ZipEntry(path + File.separator + "MessageContentPDF.html"));
	...
	public List<SimpleDataInstanceTestPlan> getTestPlans() {
